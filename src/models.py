...
def run_full_pipeline(seed: int = RANDOM_STATE) -> dict:
    ...
    logger.info("Comparaison des stratégies de déséquilibre")
    imbalance_df = compare_imbalance_strategies(X_train_proc, y_train, X_test_proc, y_test)
    ...
    if best_strategy == "smote":
        train_kwargs = {"resampler": "smote"}
        X_train_eff, y_train_eff = SMOTE(random_state=seed).fit_resample(X_train_proc, y_train)
        class_weight_use = None
    elif best_strategy == "rus":
        train_kwargs = {"resampler": "rus"}
        X_train_eff, y_train_eff = RandomUnderSampler(random_state=seed).fit_resample(X_train_proc, y_train)
        class_weight_use = None
    elif best_strategy == "class_weight":
        train_kwargs = {"resampler": "class_weight"}
        X_train_eff, y_train_eff = X_train_proc, y_train
        class_weight_use = "balanced"
    else:
        train_kwargs = {"resampler": "none"}
        X_train_eff, y_train_eff = X_train_proc, y_train
        class_weight_use = None
    ...
    if class_weight_use == "balanced":
        estimators["random_forest"].set_params(class_weight="balanced")
    ...
    final_rows = []
    artifacts: dict[str, TrainedArtifact] = {}
    for name, est in estimators.items():
        art = evaluate_estimator(name, est, X_train_eff, y_train_eff, X_test_proc, y_test)
        final_rows.append({"model": name, **art.metrics})
        artifacts[name] = art
    ...
    winner = artifacts[winner_name]
    logger.info("Modèle gagnant : %s", winner_name)

    if hasattr(winner.estimator, "predict_proba"):
        y_proba = winner.estimator.predict_proba(X_test_proc)[:, 1]
    else:
        y_proba = winner.estimator.decision_function(X_test_proc)
        y_proba = (y_proba - y_proba.min()) / (y_proba.max() - y_proba.min() + 1e-9)

    best_threshold = find_best_threshold(y_test.values, y_proba, metric="f1")
    y_pred_thr = (y_proba >= best_threshold).astype(int)
    metrics_final = compute_metrics(y_test.values, y_pred_thr, y_proba)
    metrics_final.update(confusion_to_dict(y_test.values, y_pred_thr))
    metrics_final["decision_threshold"] = best_threshold

    save_confusion_matrix(
        y_test.values,
        y_pred_thr,
        FIGURES_DIR / "confusion_matrix_final.png",
        title=f"Matrice de confusion — {winner_name} (seuil={best_threshold:.2f})",
    )
    save_roc_curve(y_test.values, y_proba, FIGURES_DIR / "roc_curve_final.png")
    save_pr_curve(y_test.values, y_proba, FIGURES_DIR / "pr_curve_final.png")
    save_threshold_plot(y_test.values, y_proba, FIGURES_DIR / "threshold_analysis.png")
